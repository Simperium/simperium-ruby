...
class TestSimperium < Test::Unit::TestCase
    ...
    def test_bucket_get
        uuid = UUID.new
        bucket = Simperium::Bucket.new(APP_ID, get_auth_token, uuid.generate(:compact))
    ...
    def test_user_get
        user = Simperium::SPUser.new(APP_ID, get_auth_token)
        user.post({'x'=> 1})
        assert_equal(user.get, {'x'=> 1})
    end

    def test_api_getitem
        api = Simperium::Api.new(APP_ID, get_auth_token)
